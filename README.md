...
- **Measured on EPIC-05 (A100, 2026-08-01)**: native backward is ~1.7-1.9x faster than the `gsplat_recompute` fallback (31.6 vs 59.9 ms on the 6.1M-Gaussian bicycle scene). Three forward optimizations closed the total-iteration gap: a batched-projection culling fix removed the per-camera full HiGS render, the differentiable forward no longer re-packs the FP16 scene per step (`pack_gaussian_inference_scene` now runs only on real topology changes), and a native compact-copy kernel (`higs_gather_visible`) replaces PyTorch's pathologically slow row gather for the visible subset (quats/colors gathers 1.7 ms each -> 0.1/0.65 ms). `higs_native` total iteration is now **faster than std gsplat end-to-end on both scenes**: 21.40 vs 22.51 ms (train, -4.9%) and 49.56 vs 51.26 ms (bicycle, -3.3%); `higs_dynamic` (densify/prune) reaches 19.07 ms (train, -15.3%) and 40.62 ms (bicycle, -20.8%) with higher held-out PSNR
