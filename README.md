...
| --- | --- | --- |
| Explain gsplat HiGS, why it is inference-only, and whether it can train | HiGS caches a packed scene, hierarchical intersections, segmented ordering, visibility masks, and persistent FP16 workspace. Its public path rejects grad-enabled calls, has no backward kernel, and is invalidated by densification/pruning. Training is possible only after adding differentiable saved/recomputed state plus backward kernels; the lowest-risk first experiment is frozen-topology late-stage training with backward recomputation. | **Analysis complete; trainable implementation not yet complete.** [Reverse engineering](docs/research-roadmap-2026.md#4-higs-reverse-engineering) · [trainable variants](docs/research-roadmap-2026.md#5-trainable-higs-variants) |
| Combine acceleration ideas from other renderers and generate new HiGS ideas | The survey covers FlashGS, Speedy-Splat, GEMM-GS, TC-GS, temporal reuse, hierarchical culling, local sorting, Tensor Cores, and compression-aware execution. The leading design is exact conic-vs-fine-tile rejection followed by a density-adaptive sparse-warp/dense-Tensor-Core blend. Thirty additional renderer hypotheses are ranked by feasibility, expected gain, risk, and novelty. | **Survey and test definitions complete; proposed HiGS fusion kernels are not yet implemented or claimed as measured speedups.** [Fusion shortlist](docs/research-roadmap-2026.md#7-higs-fusion-opportunities) · [30 research ideas](docs/research-roadmap-2026.md#8-thirty-renderer-research-ideas) · [EPIC-05 acceptance matrix](benchmark/renderer-research-experiments.json) |
| Find the highest practical near-lossless storage compression for native PLY checkpoints | SPZ v4 with 8/8-bit SH is the current measured same-checkpoint recommendation: **4.161 GB to 725.9 MB (5.732x)** over five scenes, with worst deltas of **-0.01455 dB PSNR, -0.000217 SSIM, and +0.000666 LPIPS**. All five numerical gates and all 100-frame-per-scene visual audits passed. An expanded 45-row EPIC-05 matrix is now comparing XZ lossless, Compressed PLY, SOG, SPZ precision points, block-float, and tile-codebook. | **SPZ 8/8 qualified; expanded comparison running.** [Full SPZ report](reports/epic05-spz-qualification-2026-07-24.md) · [candidate registry](benchmark/compression-candidates.json) · [compression survey](docs/research-roadmap-2026.md#9-compression-survey) · [measured artifacts](results/measured-compression/README.md) |
