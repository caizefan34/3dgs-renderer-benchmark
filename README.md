...
- **Measured on EPIC-05 (A100, 2026-08-01)**: native backward is ~2.0-2.2x faster than the `gsplat_recompute` fallback (11.9 vs 24.6 ms train / 26.6 vs 60.0 ms bicycle). Four forward optimizations (batched-projection culling, per-step FP16 pack skip, native `higs_gather_visible` compact-copy) and three backward rounds (SH VJP visible-pair compaction with channel-adjacent threads, blend VJP `__launch_bounds__(256, 5)` register-budget fix matching std's 48-register kernel, C++ direct-master gradient scatter that removed the Python `index_copy_` scatter) took the native backward to -12.5% / -19.1% vs std's own backward (11.9 vs 13.6 ms train / 26.6 vs 32.9 ms bicycle). `higs_native` total iteration is **faster than std gsplat end-to-end on both scenes**: 19.2 vs 21.7 ms (train, -11.5%) and 44.6 vs 51.1 ms (bicycle, -12.7%); `higs_dynamic` (densify/prune) reaches 17.1 ms (train, -21%) and 36.3 ms (bicycle, -29%) with higher held-out PSNR
