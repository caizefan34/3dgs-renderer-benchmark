...
- **Measured on EPIC-05 (A100, 2026-08-01)**: native backward is ~2.1x faster than the `gsplat_recompute` fallback (28.3 vs 59.9 ms on the 6.1M-Gaussian bicycle scene). Three forward optimizations closed the total-iteration gap (batched-projection culling, per-step FP16 pack skip, native `higs_gather_visible` compact-copy), and a fourth-round backward optimization (SH VJP visible-pair compaction with gsplat-style channel-adjacent thread order, plus `index_copy_` instead of `index_add_`) took the native backward to -14% vs std's own backward on bicycle (28.3 vs 32.8 ms). `higs_native` total iteration is now **faster than std gsplat end-to-end on both scenes**: 20.4 vs 21.6 ms (train, -5.6%) and 46.3 vs 50.8 ms (bicycle, -8.9%); `higs_dynamic` (densify/prune) reaches 18.3 ms (train, -15.3%) and 38.1 ms (bicycle, -25.0%) with higher held-out PSNR
