...
| Combine acceleration ideas from other renderers and generate new HiGS ideas | The survey covers FlashGS, Speedy-Splat, GEMM-GS, TC-GS, temporal reuse, hierarchical culling, local sorting, Tensor Cores, and compression-aware execution. Source review confirms HiGS already performs fine-tile conic overlap masking, so current work targets scheduling and dense-mask execution instead of duplicating that feature. | **Thirty-three ideas are registered. Three new Python-level adapters (no CUDA changes needed) are now available: adaptive half-resolution (0.5x), adaptive quarter-resolution (0.25x), and temporal frame cache. These document the extreme-speed end of the Pareto frontier. Measurements on EPIC-05 are pending.** [P99-LPT report](reports/epic05-higs-p99-lpt-2026-07-25.md) · [calibrated-selector report](reports/epic05-higs-calibrated-2026-07-24.md) · [fusion shortlist](docs/research-roadmap-2026.md#7-higs-fusion-opportunities) · [30 research ideas](docs/research-roadmap-2026.md#8-thirty-renderer-research-ideas) · [EPIC-05 acceptance matrix](benchmark/renderer-research-experiments.json) · [new adapters](src/renderers/gsplat_renderer.py) |
| Find the highest practical near-lossless storage compression for native PLY checkpoints | The completed 45-row same-checkpoint comparison covers XZ, block-float, tile-codebook, Compressed PLY, SOG, SPZ (three precision points), and SOGS. SPZ v4 8/8 passes every five-scene numerical and visual gate: **4.161 GB to 725.9 MB (5.732x)**. FCGS reproduced as a separate codec track at 12.843x (2/5 passes). SOGS (v1.0.0) installed and tested but is a one-way PlayCanvas viewer codec with no decompress API -- not suitable for benchmark round-tripping. | **SPZ 8/8 remains the near-lossless winner. FCGS offers higher compression at lower quality. SOGS is one-way only.** [Final comparison](reports/epic05-expanded-compression-qualification-2026-07-24.md) · [FCGS report](reports/epic05-fcgs-five-scene-2026-07-24.md) · [candidate registry](benchmark/compression-candidates.json) |
