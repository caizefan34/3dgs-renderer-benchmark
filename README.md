...
- **Measured on EPIC-05 (A100, 2026-08-01)**: native backward is ~1.7-1.9x faster than the `gsplat_recompute` fallback (31.8 vs 60.1 ms on the 6.1M-Gaussian bicycle scene). A batched-projection culling fix removed the per-camera full HiGS render from the differentiable forward: `higs_native` total iteration is now at std parity on the small scene (22.85 vs 22.40 ms) and +8.8% on the large scene (55.65 vs 51.13 ms); `higs_dynamic` (densify/prune) is now **faster than std gsplat end-to-end on both scenes** (20.20 vs 22.40 ms, -9.8%; 44.84 vs 51.13 ms, -12.3%) with higher held-out PSNR
