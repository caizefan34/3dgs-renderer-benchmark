...
- **Measured on EPIC-05 (A100, 2026-08-01)**: native backward is ~1.7-1.9x faster than the `gsplat_recompute` fallback (31.9 vs 60.1 ms on the 6.1M-Gaussian bicycle scene). Two forward optimizations closed the total-iteration gap: a batched-projection culling fix removed the per-camera full HiGS render, and the differentiable forward no longer re-packs the FP16 scene per step (`pack_gaussian_inference_scene` now runs only on real topology changes). `higs_native` total iteration: 22.41 vs 22.00 ms (train, +1.9%) and 52.79 vs 50.99 ms (bicycle, +3.5%); `higs_dynamic` (densify/prune) is **faster than std gsplat end-to-end on both scenes** (19.95 vs 22.00 ms, -9.3%; 43.04 vs 50.99 ms, -15.6%) with higher held-out PSNR
