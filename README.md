...
- 🚧 **Next** — close the high-N scene (garden/bicycle) quality bound at ≥1.8x; sampling-side levers are closed (alpha sweep and densify grad-accum both negative, [round-43/44](results/higs-round43/r43-summary.json)), next probes: LPIPS-weight 0.2 on the loss side (round-46, running) and --anchor-densify full-res densify at the original 5-step cadence (round-47 diagnostic), then multi-resolution matrix and M6 baseline comparisons (ICCV random-tile loss, Turbo-GS, Speedy-Splat).
