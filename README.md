...
flowchart LR
    ...
    C --> D["渐进分辨率 × 可见性衰减<br/>4/5 场景：更快 且 质量更好"]
```
...
- ✅ **可训练 HiGS 加速收官（R60–R65）** — 4/5 场景找到了「更快 且 质量更好」的严格支配配置（bicycle 训练时间 -21.7% / PSNR +0.33，garden -8.6% / PSNR +0.12 等，[§15 结果速查](reports/higs-training-speedup-research-2026-08-03.md)；train 低 N 不推荐启用）；一键复现入口：`--higs-quality-max`。
