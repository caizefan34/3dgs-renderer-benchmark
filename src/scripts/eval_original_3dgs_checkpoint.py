#!/usr/bin/env python3
"""Score one official 3DGS checkpoint on the official test split.

Runs inside the pinned graphdeco-inria/gaussian-splatting checkout (via
--source-dir) so the metrics are computed with the official implementations:
...
def main() -> int:
    ...
    if not (source_dir / "train.py").is_file():
        raise SystemExit(f"missing pinned official trainer in {source_dir}")
    ...
    saved_argv = sys.argv
    sys.argv = [
        "eval_original_3dgs_checkpoint.py",
        "-m",
