#!/usr/bin/env python
"""Run the executable Speedy-Splat subset of the HiGS paper protocol on 8 GPUs.

Each GPU runs one audited from-SfM 30k job at a time through the official
speedy-splat train.py. Per-job artifacts land under
--run-root/speedy_splat/<scene>/s<seed>; protocol-valid result JSONs are
...
RUNNER = ROOT / "benchmark" / "run_speedy_splat_training.py"
...
def _runner_command(args, job: dict, gpu: int, smoke_steps: int | None) -> list[str]:
    command = [
        ...
        "--method", "speedy_splat",
        "--scene", job["scene"],
        "--source-dir", str(args.source_speedy.resolve()),
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--source-speedy", type=Path, required=True)
    parser.add_argument(
        "--run-root", type=Path, default=ROOT / "artifacts" / "training-speedy" / "runs"
    )
    parser.add_argument(
        "--result-root", type=Path, default=ROOT / "artifacts" / "training-speedy" / "results"
    )
    parser.add_argument(
        "--session", type=Path, default=ROOT / "artifacts" / "training-speedy" / "session.json"
    )
