#!/usr/bin/env python
"""Assemble protocol-valid result JSONs from one audited official 3DGS run dir.

The official train.py saves point_cloud/iteration_<N>/point_cloud.ply at the
eval steps; the runner writes stats/val_step{step-1}.json (official PSNR/SSIM/
LPIPS) and stats/train_step{step-1}_rank0.json (wall time at the save marker,
peak memory) plus paper-run-metadata.json. This assembler converts those into
the same result contract validated by validate_higs_paper_results.py.
"""
...
def _linear_ttq(curve, threshold_psnr: float) -> float:
    ...
    prev_p = prev_w = None
    for point in curve:
        w, p = point["wall_time_seconds"], point["psnr_db"]
        if p >= threshold_psnr:
...
def assemble(run_dir: Path, job: dict, *, gpu_index: int, energy_joules: float = 0.0) -> dict:
    ...
    for index, step in enumerate(curve_steps):
        ...
        if index == len(curve_steps) - 1:
            ...
            train = _load(run_dir / "stats" / f"train_step{step - 1:04d}_rank0.json")
    ...
    peak_mib = float(
        resources.get("peak_gpu_memory_mib") or (float(train_final["mem"]) * 1024.0)
    )
    gaussian_count = int(final["num_GS"])

    final_ply = run_dir / "point_cloud" / f"iteration_{iterations}" / "point_cloud.ply"
    if not final_ply.is_file():
        raise ValueError(f"missing final checkpoint PLY: {final_ply}")
    digest = hashlib.sha256()
    with final_ply.open("rb") as handle:
    ...
    result = {
        ...
        "resources": {
            ...
            "energy_joules": float(resources.get("energy_joules", energy_joules)),
            "final_gaussian_count": gaussian_count,
        },
        "quality_curve": curve,
        "artifact": {
            "path": final_ply.relative_to(run_dir).as_posix(),
