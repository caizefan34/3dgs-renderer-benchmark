#!/usr/bin/env python3
"""
R3 Certificate Tightness Gate 闁?Corrected Runner
...
# ============================================================
# SepSSIM 闁?identical to canonical baseline
# ============================================================
...
def compute_C_max_t(colors_rgb, tile_offsets, flatten_ids, tile_h, tile_w):
    """
    C_max_t = max_{j in G_t} ||c_j||_2 over per-Gaussian SH color norms per tile.

    R3.1-B FIX: uses true SH-evaluated color norms (||color_j||_2),
    not the old conic-norm proxy.
    """
    device = colors_rgb.device
    ...
    for tile_idx in range(n_tiles):
        ...
        if end > start:
            ...
            C_max_t[tile_idx] = colors_rgb[0, g_unique].norm(dim=-1).max().item()
...
def _validate_work_weights(wc_vals, wu_vals, tile_gaussian_total, tile_size):
    ...
    if len(wc_nonzero) > 0:
        ...
        if all_ones:
            print("    *** WARNING: all nonzero W_color == 1 闁?faithful replay may still be broken! ***")
        elif np.mean(wc_nonzero == 1.0) > 0.95:
            print("    *** WARNING: >95% of nonzero W_color == 1 闁?likely still using torch.unique count! ***")
...
def _accumulate_tile_bounds(opacities, conics, means2d, colors_rgb, tile_offsets,
                            flatten_ids, Q_t, tile_h, tile_w, tile_size,
                            H=None, W=None,
                            uniform_families=False):
    """Compute ALL certificate bounds — fully vectorized per-tile.

    R3.1-B FIX: colors_rgb [1, G, 3] (SH-evaluated RGB) is now required.
    The color norm ||c_i||_2 replaces the old conic-norm proxy.
    """
    device = opacities.device
    n_tiles = tile_h * tile_w
    N = opacities.shape[0]
    ...
    for tile_idx in range(n_tiles):
        ...
        # ---- Faithful W_it replay 閳?tensor version ----
        ...
        # R3.1-B FIX: use true SH-evaluated color norm, not conic norm proxy
        ...
        # R3.1-B FIX: C_max_t from color norms, not conic norms
        C_max_t_val = float(color_norm_batch.max().item()) if K > 0 else 0.0

        # ================================================================
        # VECTORIZED bound computation
        # ================================================================
        o_j = opac_batch
        c_norm = color_norm_batch  # R3.1-B: true ||c_i||_2 from SH
        s_min_v = sigma_min_v
        ...
        if K > 0:
            ...
            batch = torch.stack([
                ...
                s_min_v, o_j, c_norm,
            ]).cpu().numpy()  # [12, K] 閳?single .cpu() call
            m2d_g_np = contrib_mean2d_global.cpu().numpy()  # [K] 閳?separate (already computed)
...
def run_r3_measurement(checkpoint_path, start_iter, n_iters, output_dir,
    ...
    for i in range(n_iters):
        ...
        with torch.no_grad():
            ...
            )  # [1, tile_h, tile_w] 闁?form for rasterize_to_pixels
        ...
        # === Compute C_max_t ===
        C_max_t = compute_C_max_t(colors_rgb.detach(), tile_offsets, flatten_ids, tile_h, tile_w)
        
        # === Certificate Bounds (C1, C2, C3, C6 all applied) ===
        bounds = _accumulate_tile_bounds(
            opacities.detach(), conics.detach(), means2d.detach(),
            colors_rgb.detach(),  # R3.1-B: pass true SH-evaluated colors
        ...
        # === Certificate semantics (red-team req 7): 閿滅保f + U_f ===
        # 閿滅保f = 閸?B_f / 閸?||g_f||  闁?bound inflation (how much wider bounds are vs actual)
...
def main():
    parser = argparse.ArgumentParser(
        description="R3 Certificate Tightness Gate 闁?Corrected Runner"
    )
