...
class _HigsAutogradFunction(torch.autograd.Function):
    ...
    def forward(
        ...
        if visible_ids is not None and visible_ids.numel() > 0:
        ...
        if sh_degree is not None and getattr(ctx, "sh_compression_mode", "none") != "none":
        ...
        if native:
            render_colors, render_alphas, captured = (
                _HigsAutogradFunction._native_forward_capture(
                    ctx, v_means_b, v_quats_b, v_scales_b, v_opacities_b,
                    v_colors_input, viewmats, Ks, width, height, sh_degree,
                    tile_size, near_plane, far_plane, radius_clip, eps2d,
