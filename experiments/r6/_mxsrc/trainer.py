...
def evaluate_model(model, dataset, ssim_fn, sh_degree, n_cameras=10):
...
def run_training(config: ReferenceV1Config, output_dir: str, allow_dirty: bool = False):
    ...
    for iteration in range(1, config.iterations + 1):
        ...
        if iteration in config.eval_iterations:
            ...
            metrics = evaluate_model(model, dataset, ssim_fn, model.active_sh_degree)
...
if __name__ == "__main__":
    ...
    run_training(config, args.output_dir, allow_dirty=args.allow_dirty)
