...
| Differentiable HiGS | Can hierarchical rendering support correct gradients and reduce end-to-end training cost without sacrificing converged quality? | Native CUDA backward, topology lifecycle, multi-seed studies, and a frozen 177-job from-scratch A100 matrix; trainability and a 21.6% mean peak-memory reduction are measured | Quality-preserving full-convergence speedup and official-baseline claims remain blocked |
...
The HiGS method is the strongest candidate for a primary method paper: the
frozen 177-job from-scratch matrix supports trainability and a memory
reduction, while the quality-preserving speed claim stays blocked by the
measured quality gap. The survey can become an artifact or survey paper after
a systematic search audit. The storage work can become an empirical study
after adding decode and deployment costs plus current learned codecs.
