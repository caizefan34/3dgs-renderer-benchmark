#!/usr/bin/env python3
"""Compile AccuTile extension directly, bypassing buggy _backend.py."""
...
sources = (
    sorted(glob.glob(os.path.join(PATH, "csrc/*.cu")))
    + sorted(glob.glob(os.path.join(PATH, "csrc/*.cpp")))
    + [os.path.join(PATH, "ext.cpp")]
)
extra_include_paths = [os.path.join(PATH, "include/"), glm_path]

print("Compiling AccuTile extension with %d sources..." % len(sources))
print("Build dir: ~/.cache/torch_extensions/py310_cu128/gsplat_cuda")
