#!/usr/bin/env python3
"""
Phase C53-Validation2 — Workload Pair Collection

Modified collect_signals.py that adds CURRENT WORKLOAD tracking:
  - tiles_sum_window: accumulated tiles_per_gauss over visibility window (~50 iters)
  - current_tiles_mean: mean tiles_per_gauss when visible (windowed average)
  - current_tiles_checkpoint: tiles_per_gauss at the exact checkpoint iteration

This enables the critical comparison:
  M1: W_current → W_future  (workload persistence)
  M2: R_current → W_future  (footprint prediction)
  M3: W_current + R_current → W_future  (incremental)

All existing C53-Discovery signals and future targets are preserved.
"""
...
class FutureWindow:
    ...
    def snapshot(self, delta, n_iters):
        self.snapshots[delta] = {
            ...
            "tiles_sum": self.tiles_sum.clone(),
...
def run_collection(dataset, sfm_data, gpu_id, scene, seed, iters=30000):
    ...
    # NEW: tiles accumulation window (current workload)
    tiles_sum_window = torch.zeros(N, device=device)
    ...
    active_window = None
    checkpoint_data = {}
    current_tiles_at_cp = None  # tiles_per_gauss at checkpoint iteration
    ...
    def extend_tracking(n_new, current_iter):
        ...
        nonlocal vis_count_window, radius_sum_window, tiles_sum_window
        ...
        tiles_sum_window = torch.cat([tiles_sum_window, torch.zeros(n_new, device=device)])
        next_id += n_new
    ...
    def filter_tracking(keep_mask):
        ...
        tiles_sum_window = tiles_sum_window[keep_mask]
    ...
    for iter_idx in range(iters):
        ...
        # Visibility and workload from current render
        ...
        # NEW: accumulate tiles_per_gauss for current workload signal
        tiles_sum_window[current_visible] += tiles_meta[current_visible].float()
        ...
        # Visibility/tiles window reset at offset-50 iterations
        if iter_idx > 0 and (iter_idx - 50) % 100 == 0:
            ...
            tiles_sum_window = torch.zeros(model.xyz.shape[0], device=device)
        ...
        if iter_idx in CHECKPOINTS:
            ...
            # NEW: current workload = mean tiles_per_gauss when visible
            tiles_signal = tiles_sum_window / vis_count_window.clamp(min=1)
            # Also record raw tiles_sum and vis_count for flexibility
            tiles_raw = tiles_sum_window
            ...
            cp_data = {
                ...
                "signals": {
                    ...
                    # NEW: current workload signals
                    "current_tiles_mean": tiles_signal[sample_pos].cpu().numpy(),
                    "current_tiles_sum": tiles_raw[sample_pos].cpu().numpy(),
                    "current_vis_count": vis_signal[sample_pos].cpu().numpy(),
                },
            }
            ...
            print(f"  [CP {iter_idx}] Sampled {n_sample}/{n_total}, "
                  ...
                  f"tiles_mean={tiles_signal[sample_pos].float().mean().item():.1f}")
...
def save_raw_data(checkpoint_data, scene, seed, save_dir):
    ...
    filepath = save_dir / f"{scene}{suffix}_workload.npz"
...
def main():
    ...
    print(f"Phase C53-Validation2 — Workload Pair Collection")
    ...
    save_dir = repo_root / "results" / "a100" / "phase-c53-validation2"
