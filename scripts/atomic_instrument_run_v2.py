#!/usr/bin/env python3
"""Direct atomic instrumentation v2 — self-contained bin_final computation.

Uses gsplat's forward pass to extract metadata (means2d, conics, opacities,
tile_offsets, flatten_ids), then runs a standalone CUDA kernel that:
  1. Computes bin_final per pixel via forward compositing
  2. Counts warp/block atomic events in backward iteration pattern
...
ATOMIC_CU = "/tmp/atomic_instrument_v2.cu"
...
def get_forward_metadata(state, cam, accutile, device="cuda"):
    """Run forward pass and extract metadata for atomic counting."""
    import gsplat
    from gsplat import rasterization, fully_fused_projection
    ...
    # Run full rasterization to get tile_offsets and flatten_ids
    ...
    tile_offsets = meta["isect_offsets"][0, 0].contiguous()
    ...
    means2d = proj_results[1][0].float().contiguous()  # [N, 2]
    conics = proj_results[3][0].float().contiguous()    # [N, 3]
...
def compile_atomic_counter():
    ...
    print("Compiling atomic instrumentation kernel v2...")
    ext = load(
        name="atomic_instrument_v2",
        sources=[ATOMIC_CU],
...
def count_atomics(ext, meta):
    ...
    n_isects = torch.zeros(1, dtype=torch.int64, device="cuda")

    ext.count_backward_atomics_v2(
        meta["means2d"], meta["conics"], meta["opacities"],
        meta["width"], meta["height"], 16,
        meta["tile_offsets"], meta["flatten_ids"],
        n_warp, n_block, n_isects,
    )
    torch.cuda.synchronize()
    return int(n_warp.item()), int(n_block.item()), int(n_isects.item())
...
def main():
    ...
    for scene, paths in SCENES.items():
        ...
        for ci in cam_indices:
            ...
            n_warp_b1, n_unique_b1, n_isect_b1 = count_atomics(ext, meta_b1)
            print(f"    B1: warp_atomics={n_warp_b1:,}, block_uniques={n_unique_b1:,}, total_isects={n_isect_b1:,}")

            n_warp_b1a, n_unique_b1a, n_isect_b1a = count_atomics(ext, meta_b1a)
            print(f"    B1A: warp_atomics={n_warp_b1a:,}, block_uniques={n_unique_b1a:,}, total_isects={n_isect_b1a:,}")

            ATOMICS_PER_EVENT = 11
            ...
            entry = {
                ...
                "kernel_verified_isects_B1": n_isect_b1,
                "kernel_verified_isects_B1A": n_isect_b1a,
            }
            if bench_b1:
    ...
    # Summary
    print(f"\n{'='*70}")
    print("DIRECT ATOMIC INSTRUMENTATION SUMMARY (v2)")
    print(f"{'='*70}")
    ...
    out_path = "/tmp/accutile_a100_results/atomic_instrumentation_v2.json"
