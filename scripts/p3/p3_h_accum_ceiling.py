...
def cmd_timing(args):
    ...
    rows = []
    for scene in scenes:
        # build fixture once per scene (backward-kind reuses it; fb re-captures)
        fx = build_fixture(scene, MAX_LONG_SIDE, device, None)
        # balanced rotating order across variants
        for rep_i in range(reps):
            order = variants[(rep_i % len(variants)):] + variants[:(rep_i % len(variants))]
            for v in order:
                ...
                for kind in ("backward", "fb"):
                    ...
                    if err:
                        rows.append({"scene": scene, "variant": v, "rep": rep_i,
                                     "kind": kind, "value_ms": None, "error": err})
                        continue
                    rows.append({"scene": scene, "variant": v, "rep": rep_i,
                                 "kind": kind, "value_ms": round(ms, 5)})
