#!/usr/bin/env python3
"""
C42 P1 Training Validation: Baseline vs Downsampled SSIM (scale=0.75).

Same protocol as P0 but scale=0.75 instead of 0.50.
Adds: densification/pruning event tracking (cloned/split/pruned per interval).
"""
...
def main():
    print("=" * 72)
    print("C42 P1 Training Validation: Baseline vs Downsampled SSIM (scale=0.75)")
    print("=" * 72)
    ...
    results_b = train_variant(dataset, sfm_data, scale=0.75, variant_name="B_downsampled_0.75")
    torch.cuda.empty_cache()

    # ── Comparison ──
    print(f"\n{'='*72}")
    print("COMPARISON: A (baseline) vs B (scale=0.75)")
    print(f"{'='*72}")
    ...
    if psnr_pass and ssim_pass and gs_pass and speedup_pass:
        decision = "PASS -- all gates met. Scale=0.75 is the primary candidate. Prepare 30K validation."
    else:
    ...
    # Save
    output = {
        "experiment": "C42 P1 Training Validation (scale=0.75)",
    ...
    save_path = repo_root / "results" / "a100" / "phase-c42" / "c42_p1_training_validation_0.75.json"
