...
Reproduces original P1 condition: prune every densification iteration.
This keeps Gaussian count around 1M, making SSIM the dominant cost.

Scales: 1.0, 0.875, 0.75, 0.625, 0.5
5K training, room scene, seed=42.

Densification schedule (matching original 3DGS):
- interval: 100 iters (iter 500 to 5000)
- prune: every densification iteration (opacity_threshold=0.005)
...
def train_scale(dataset, sfm_data, scale, total_iters=TOTAL_ITERS):
    ...
    for iter_idx in range(total_iters):
        ...
        # Densification with CONTINUOUS PRUNING (matching original 3DGS)
        if iter_idx >= DENSIFY_START and iter_idx < DENSIFY_END and iter_idx % DENSIFY_INTERVAL == 0:
            ...
            # CRITICAL: Prune every densification iteration
        ...
        if iter_idx % EVAL_INTERVAL == 0 or iter_idx == total_iters - 1:
            ...
            print(f"  [s={scale}@{iter_idx}] PSNR={psnr:.2f} SSIM={ssim_v:.4f} LPIPS={lpips_v:.4f} GS={gs_count:,} (pruned={total_prune:,})")
...
def main():
    ...
    print("=" * 72)
    print(f"Track 0: C42 Re-validation with continuous pruning (scale={args.scale})")
    print("=" * 72)
    print(f"  GPU: {torch.cuda.get_device_name(0)}  Scale: {args.scale}  Iters: {TOTAL_ITERS}")
    print(f"  Pruning: every {DENSIFY_INTERVAL} iters (threshold={PRUNE_THRESHOLD})")
    ...
    save_path = repo_root / "results" / "a100" / "phase-c42" / f"track0_pruned_{scale_str}.json"
