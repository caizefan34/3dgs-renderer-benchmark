#!/usr/bin/env python3
"""
C42 P2 Training Validation (30K): Baseline vs Downsampled SSIM (scale=0.75).

Full 30K-iteration training validation on A100-PCIE-40GB.
Same protocol as P1 but 30K iterations instead of 10K.
Adds: checkpoint saving (10K/20K/30K), phase-based time breakdown, eval overhead tracking.
"""
...
DENSIFICATION_PHASE_END = 15000  # iter boundary for phase timing
...
def train_variant(dataset, sfm_data, scale, variant_name, save_dir):
    print(f"\n{'='*60}")
    ...
    # Phase timing
    phase_dens_iter_times = []   # iter 1 to DENSIFICATION_PHASE_END
    phase_post_iter_times = []   # iter DENSIFICATION_PHASE_END+1 to NUM_ITERS
    ...
    for iteration in range(1, NUM_ITERS + 1):
        ...
        if iteration % CHECKPOINT_INTERVAL == 0:
            ...
            ckpt_path = save_dir / f"{variant_name}_iter{iteration}.pt"
            torch.save(ckpt, ckpt_path)
            ckpt_eval_psnr, ckpt_eval_ssim = psnr, ssim  # last eval values
            results["checkpoints"].append({
                "iter": iteration,
                "path": str(ckpt_path),
                "n_gaussians": model.xyz.shape[0],
                "psnr_at_last_eval": ckpt_eval_psnr,
                "ssim_at_last_eval": ckpt_eval_ssim,
            })
            print(f"  [ckpt@{iteration}] saved to {ckpt_path.name}  GS={model.xyz.shape[0]:,}")
    ...
    results["timing"] = {
        ...
        "eval_overhead_pct": total_eval_time_s / total_time * 100,
    ...
    results["phase_timing"] = {
        "densification_phase": {
            ...
            "std_iter_ms": float(np.std(phase_dens_iter_times)) if phase_dens_iter_times else 0,
        ...
        "post_densification_phase": {
            ...
            "std_iter_ms": float(np.std(phase_post_iter_times)) if phase_post_iter_times else 0,
    ...
    print(f"  Train only: {train_time_s:.1f}s  Eval overhead: {total_eval_time_s:.1f}s ({total_eval_time_s/total_time*100:.1f}%)")
    print(f"  Mean iter: {np.mean(iter_times):.2f} ms")
    print(f"  Dens phase mean: {np.mean(phase_dens_iter_times):.2f} ms  Post-dens mean: {np.mean(phase_post_iter_times):.2f} ms")
    print(f"  Cumulative: cloned={cumulative_cloned:,}  split={cumulative_split:,}  pruned={cumulative_pruned:,}")
...
def main():
    print("=" * 72)
    print("C42 P2 Training Validation (30K): Baseline vs Downsampled SSIM (scale=0.75)")
    print("=" * 72)
    ...
    save_dir = repo_root / "results" / "a100" / "phase-c42" / "p2_checkpoints"
    ...
    results_a = train_variant(dataset, sfm_data, scale=1.0, variant_name="A_baseline", save_dir=save_dir)
    torch.cuda.empty_cache()

    # ── Run Variant B (scale=0.75) ──
    results_b = train_variant(dataset, sfm_data, scale=0.75, variant_name="B_downsampled_0.75", save_dir=save_dir)
    torch.cuda.empty_cache()

    # ── Comparison ──
    print(f"\n{'='*72}")
    print("COMPARISON: A (baseline) vs B (scale=0.75) -- 30K iterations")
    print(f"{'='*72}")
    ...
    print(f"    Speed: A={results_a['timing']['mean_iter_ms']:.1f}ms  B={results_b['timing']['mean_iter_ms']:.1f}ms  "
          f"speedup={speedup:+.1f}%  (gate: > 30%)")

    # Time breakdown
    ta_t, tb_t = results_a["timing"], results_b["timing"]
    ta_p, tb_p = results_a["phase_timing"], results_b["phase_timing"]
    print(f"\n  TIME BREAKDOWN:")
    print(f"    Total wall:  A={ta_t['total_wall_s']:.0f}s ({ta_t['total_wall_min']:.1f}min)  "
          f"B={tb_t['total_wall_s']:.0f}s ({tb_t['total_wall_min']:.1f}min)  "
          f"saved={ta_t['total_wall_s']-tb_t['total_wall_s']:.0f}s")
    print(f"    Train only:  A={ta_t['train_only_s']:.0f}s  B={tb_t['train_only_s']:.0f}s")
    print(f"    Eval overhead: A={ta_t['eval_overhead_s']:.0f}s ({ta_t['eval_overhead_pct']:.1f}%)  "
          f"B={tb_t['eval_overhead_s']:.0f}s ({tb_t['eval_overhead_pct']:.1f}%)")
    print(f"    Dens phase mean:  A={ta_p['densification_phase']['mean_iter_ms']:.1f}ms  "
          f"B={tb_p['densification_phase']['mean_iter_ms']:.1f}ms")
    print(f"    Post-dens mean:   A={ta_p['post_densification_phase']['mean_iter_ms']:.1f}ms  "
          f"B={tb_p['post_densification_phase']['mean_iter_ms']:.1f}ms")

    # Topology comparison
    ta, tb = results_a["cumulative_topology"], results_b["cumulative_topology"]
    print(f"\n  CUMULATIVE TOPOLOGY EVENTS:")
    print(f"    Cloned:  A={ta['total_cloned']:,}  B={tb['total_cloned']:,}  d={tb['total_cloned']-ta['total_cloned']:+,}")
    print(f"    Split:   A={ta['total_split']:,}  B={tb['total_split']:,}  d={tb['total_split']-ta['total_split']:+,}")
    print(f"    Pruned:  A={ta['total_pruned']:,}  B={tb['total_pruned']:,}  d={tb['total_pruned']-ta['total_pruned']:+,}")

    # Decision
    psnr_pass = d_psnr > -0.2
    ssim_pass = d_ssim > -0.005
    gs_pass = abs(d_gs) < 10
    speedup_pass = speedup > 30
    ...
    print(f"    Speedup > 30%:         {'PASS' if speedup_pass else 'FAIL'} ({speedup:+.1f}%)")
    ...
    # Save
    output = {
        "experiment": "C42 P2 Training Validation (30K, scale=0.75)",
        ...
        "variant_B_downsampled_0.75": results_b,
        "comparison": {
