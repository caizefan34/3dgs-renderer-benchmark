#!/usr/bin/env python3
"""Check if 50-step training on bicycle is VRAM-feasible."""
...
# Load camera + GT image
print("Loading cameras + GT...")
...
print(f"N={N:,} Gs, free VRAM={torch.cuda.mem_get_info()[0]/1024**3:.2f}GB")

# Run 10 steps (no densification)
...
for step in range(10):
    ...
    rendered, alpha, meta = rasterization(
        ...
        tile_size=16, packed=True, sh_degree=3,
    )
    ...
    if step == 0:
        ...
        print(f"  Step {step}: loss={loss.item():.4f}, peak={peak:.2f}GB, alloc={alloc:.2f}GB")

free_after = torch.cuda.mem_get_info()[0]/1024**3
peak = torch.cuda.max_memory_allocated()/1024**3
print(f"\nResult: peak={peak:.2f}GB, free={free_after:.2f}GB")

# Add second camera to simulate full training
cam1 = real_cameras[1]
for attr in ["viewmatrix", "projmatrix", "camera_center", "world_view_transform", "full_proj_transform", "K"]:
    t = getattr(cam1, attr)
    if isinstance(t, torch.Tensor):
        setattr(cam1, attr, t.to(DEVICE))
rv2 = cam1.viewmatrix.unsqueeze(0)
rK2 = cam1.K.unsqueeze(0)
...
print(f"\nFree VRAM after warmup: {torch.cuda.mem_get_info()[0]/1024**3:.2f}GB")
torch.cuda.empty_cache()
gc.collect()
print(f"After cache clear: {torch.cuda.mem_get_info()[0]/1024**3:.2f}GB")

if peak < 7.0:
    print("\nCONCLUSION: 500-step training FEASIBLE on 8GB GPU")
else:
    print(f"\nCONCLUSION: 500-step training RISKY (peak={peak:.2f}GB on 8GB GPU)")
