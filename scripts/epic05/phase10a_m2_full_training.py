...
Runs 30K-step real-GT full training for packed=True vs packed=False
with complete pipeline (densification, pruning, L1 + D-SSIM loss,
Adam optimizer state preservation across topology changes).

Config matches Phase 9C methodology (room scene, 1080p, tile_size=16, SH=3).
Only packed/dense mode varies.

All configs are controlled:
  - same initialization (same SfM point cloud)
  - same GT (render from SH3 reference)
  - same camera sequence (first camera, as in Phase 9C)
  - same random seed
  - same optimizer (Adam with Phase 9C LR groups)
  - same loss (L1 + D-SSIM, lambda=0.2)
  - same SH degree (3)
  - same densification (start=500, end=15000, interval=100, grad_threshold=0.0002)
  - same pruning (interval=100, prune_opacity=0.005)
  - same checkpoint policy (every 5000 steps)
  - same number of iterations (30000)

Output:
  - results/epic05/phase10a/m2_packed_full_training.json
  - results/epic05/phase10a/m2_dense_full_training.json
  - results/epic05/phase10a/phase10a_m2_full_training_results.json (combined)
  - reports/epic05/phase10a_m2_full_training.md
"""
...
class GaussianModel:
    ...
    def __init__(self, scene_data, sh_degree=3, device="cuda"):
    ...
    def create_optimizer(self):
        """Create Adam optimizer preserving state for surviving params."""
        old_state = None
    ...
    def render(self, camera, packed=True):
...
def train_m2_mode(scene_data, cameras, packed, n_steps=30000, device="cuda",
                  output_dir=None, save_interval=5000):
    """Full 30K training for one packed/dense mode."""
    first_cam = cameras[0]
    mode_label = "packed" if packed else "dense"

    log_prefix = f"  [{mode_label}]"
    print(f"\n{'='*70}")
    print(f"  FULL 30K TRAINING  {mode_label.upper()}  (packed={packed})")
    print(f"{'='*70}")

    # GT: render from SH3 reference (using packed=True for GT — produces identical output)
    print(f"{log_prefix} Rendering SH3 reference GT...")
    from gsplat import rasterization
    sd = load_ply(str(PROJECT_ROOT / "data" / "official" / "mipnerf360" / "room" / "point_cloud.ply"), device=device)
    scales_act = torch.exp(sd["scales"].to(device))
    opacities_act = torch.sigmoid(sd["opacity"].to(device))
    with torch.no_grad():
        ref_img, _, _ = rasterization(
            means=sd["xyz"].to(device), quats=sd["rotations"].to(device),
            scales=scales_act, opacities=opacities_act,
            colors=sd["shs"].to(device),
            viewmats=first_cam.viewmatrix.unsqueeze(0),
            Ks=first_cam.K.unsqueeze(0),
            width=first_cam.image_width, height=first_cam.image_height,
            sh_degree=3, packed=True, render_mode="RGB",
        )
        gt_image = ref_img[0].clamp(0, 1)
    gt_c = gt_image.unsqueeze(0).permute(0, 3, 1, 2)
    del sd, ref_img
    torch.cuda.empty_cache()

    # Build model
    model = GaussianModel(scene_data, 3, device)
    model.create_optimizer()
    n_initial = model.n
    print(f"{log_prefix} Init Gs: {n_initial:,}  SH=3  packed={packed}")
    ...
    for step in range(n_steps):
        ...
        rendered = model.render(first_cam, packed=packed)
        torch.cuda.synchronize()
        ...
        if (step + 1) % 500 == 0:
            ...
            avg_fwd = np.mean(track["fwd_ms"][-500:])
            avg_bwd = np.mean(track["bwd_ms"][-500:])
            avg_opt = np.mean(track["opt_ms"][-500:])
            avg_topo = np.mean(track["topo_ms"][-500:])
            print(f"  Step {step+1:5d}/{n_steps}  loss={loss.item():.4f}  "
                  f"PSNR={psnr:.2f}  Gs={model.n:,}  "
                  f"iter={avg_iter:.0f}ms  (fwd={avg_fwd:.1f} bwd={avg_bwd:.1f} "
                  f"opt={avg_opt:.1f} topo={avg_topo:.1f})  "
                  f"VRAM={mem:.0f}MB", flush=True)

        # ── Checkpoint ──
        if output_dir and (step + 1) % save_interval == 0:
            ckpt = dict(
                packed=packed, step=step + 1,
            ...
            torch.save(ckpt, output_dir / f"m2_{mode_label}_step{step+1:05d}.pt")
    ...
    if output_dir:
        ...
        torch.save(ckpt, output_dir / f"m2_{mode_label}_final.pt")

    # Time-to-target
    time_to_target = {}
    for target in [25.0, 26.0, 27.0, 28.0, 29.0, 30.0, 31.0]:
...
def main():
    ...
    parser.add_argument("--only", choices=["packed", "dense", "both"], default="both")
    ...
    output_dir = Path(args.output_dir) if args.output_dir else \
        PROJECT_ROOT / "results" / "epic05" / "phase10a"
    output_dir.mkdir(parents=True, exist_ok=True)

    scene_data, cameras = load_scene(device)
    print(f"Scene: {args.scene} ({scene_data['num_points']:,} Gs, {len(cameras)} cameras)")

    modes_to_run = []
    if args.only == "both":
        modes_to_run = [("packed", True), ("dense", False)]
    else:
        modes_to_run = [(args.only, args.only == "packed")]

    all_results = {}
    for label, packed in modes_to_run:
        torch.cuda.empty_cache()
        result = train_m2_mode(
            scene_data, cameras, packed, args.steps, device,
            output_dir, args.save_interval)
        all_results[label] = result

        with open(output_dir / f"m2_{label}_full_training.json", "w") as f:
            json.dump(result, f, indent=2)

    combined = dict(
        experiment_id="phase10a-m2-full-training",
        date=datetime.now(timezone.utc).strftime("%Y-%m-%d"),
        hardware=torch.cuda.get_device_name(0) if torch.cuda.is_available() else "cpu",
        scene=args.scene, resolution="1920x1080",
        n_steps=args.steps,
        config=dict(
            ...
            tile_size=16, sh_degree=3,
        ),
        results=all_results,
    )
    with open(output_dir / "phase10a_m2_full_training_results.json", "w") as f:
        json.dump(combined, f, indent=2)

    # Summary table
    print(f"\n{'='*70}")
    print(f"  PHASE 10A — M2 Packed vs Dense Full 30K Training Summary")
    print(f"{'='*70}")
    print(f"  {'Mode':>8} | {'Final PSNR':>10} | {'Best PSNR':>10} | {'Time':>8} | "
          f"{'Gs Final':>10} | {'Iter/s':>7} | {'Peak VRAM':>9}")
    print(f"  {'-'*8}-+-{'-'*10}-+-{'-'*10}-+-{'-'*8}-+-{'-'*10}-+-{'-'*7}-+-{'-'*9}")
    for label in ["packed", "dense"]:
        if label in all_results:
            r = all_results[label]
            print(f"  {label:>8} | {r['final_psnr']:>8.2f} dB | {r['best_psnr']:>8.2f} dB | "
                  f"{r['total_time_min']:>6.1f}m | {r['n_final']:>10,} | "
                  f"{r['iter_per_sec']:>6.2f} | {r['peak_vram_mb']:>7.0f}MB")

    # Comparison section
    if "packed" in all_results and "dense" in all_results:
        p = all_results["packed"]
        d = all_results["dense"]
        print(f"\n  {'='*70}")
        print(f"  COMPARISON")
        print(f"  {'='*70}")
        print(f"  Wall time:       packed {p['total_time_min']:6.1f}m   dense {d['total_time_min']:6.1f}m   "
              f"ratio {d['total_time_s']/p['total_time_s']:.3f}x (dense/packed)")
        print(f"  Avg iter:        packed {p['avg_iter_ms']:6.1f}ms  dense {d['avg_iter_ms']:6.1f}ms")
        print(f"  Avg fwd:         packed {p['avg_fwd_ms']:6.1f}ms  dense {d['avg_fwd_ms']:6.1f}ms  "
              f"ratio {d['avg_fwd_ms']/p['avg_fwd_ms']:.3f}x (dense/packed)")
        print(f"  Avg bwd:         packed {p['avg_bwd_ms']:6.1f}ms  dense {d['avg_bwd_ms']:6.1f}ms  "
              f"ratio {p['avg_bwd_ms']/d['avg_bwd_ms']:.3f}x (packed/dense)")
        print(f"  Avg opt:         packed {p['avg_opt_ms']:6.1f}ms  dense {d['avg_opt_ms']:6.1f}ms")
        print(f"  Avg topo:        packed {p['avg_topo_ms']:6.1f}ms  dense {d['avg_topo_ms']:6.1f}ms")
        print(f"  Best PSNR:       packed {p['best_psnr']:6.2f}dB  dense {d['best_psnr']:6.2f}dB  "
              f"delta {p['best_psnr']-d['best_psnr']:+.4f}dB")
        print(f"  Final Gs:        packed {p['n_final']:>10,}  dense {d['n_final']:>10,}  "
              f"delta {p['n_final']-d['n_final']:+,}")
        print(f"  Peak VRAM:       packed {p['peak_vram_mb']:6.0f}MB  dense {d['peak_vram_mb']:6.0f}MB")
        print(f"  NaN/Inf:         packed {p['nan_detected']}  dense {d['nan_detected']}")

        # Time-to-target comparison
        print(f"\n  Time-to-Target:")
        print(f"  {'Target':>12} | {'Packed Step':>11} | {'Packed Time':>12} | "
              f"{'Dense Step':>11} | {'Dense Time':>12} | {'Winner':>8}")
        print(f"  {'-'*12}-+-{'-'*11}-+-{'-'*12}-+-{'-'*11}-+-{'-'*12}-+-{'-'*8}")
        for tgt_key in ["PSNR_25.0dB", "PSNR_26.0dB", "PSNR_27.0dB", "PSNR_28.0dB", "PSNR_29.0dB", "PSNR_30.0dB"]:
            pt = p["time_to_target"].get(tgt_key, {})
            dt = d["time_to_target"].get(tgt_key, {})
            p_step = pt.get("step", "-")
            p_time = f"{pt.get('time_seconds', '-')}s" if pt.get("reached") else "NOT_REACHED"
            d_step = dt.get("step", "-")
            d_time = f"{dt.get('time_seconds', '-')}s" if dt.get("reached") else "NOT_REACHED"

            if pt.get("reached") and dt.get("reached"):
                if pt["time_seconds"] < dt["time_seconds"]:
                    winner = "packed"
                elif dt["time_seconds"] < pt["time_seconds"]:
                    winner = "dense"
                else:
                    winner = "tie"
            elif pt.get("reached"):
            ...
            tgt = tgt_key.replace("PSNR_", "").replace("dB", " dB")
            print(f"  {tgt:>12} | {str(p_step):>11} | {p_time:>12} | "
                  f"{str(d_step):>11} | {d_time:>12} | {winner:>8}")

    print(f"\nResults: {output_dir / 'phase10a_m2_full_training_results.json'}")
