...
inline __device__ bool decode_gaussian_conic(
    const float *__restrict__ conics, const float *__restrict__ means2d, int32_t idx, GaussianConic &gc
)
{
    const float4 conics_opac = reinterpret_cast<const float4 *>(conics)[idx]; // {l0,l1,l2,opacity} fp32
    const float l0           = conics_opac.x;
    const float l1           = conics_opac.y;
    const float l2           = conics_opac.z;
    const float opacity      = conics_opac.w;
