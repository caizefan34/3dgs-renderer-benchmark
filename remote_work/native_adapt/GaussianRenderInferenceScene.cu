...
namespace gaussian_render_inference_scene
{
    ...
    )
    {
        ...
        state->conics         = at::empty({1, 1, num_gaussians, 4}, opts_f);
        state->colors         = at::zeros({num_gaussians, 4}, opts_f);

        // ---- Pre-allocate default black background: [R, G, B, T] ----
        state->background = at::zeros({1, 4}, opts_f);
        state->background.select(1, 3).fill_(1.0f);
    ...
    )
    {
        ...
        if(out_rgbt.has_value())
        {
            ...
            TORCH_CHECK(buf.dtype() == at::kFloat, "out_rgbt must be float32");
        ...
        if(state.num_gaussians == 0)
        {
            at::Tensor rgbt = out_rgbt.has_value() ? out_rgbt.value() : at::zeros({1, height, width, 4}, opts_f);
            rgbt.zero_();
            rgbt.select(3, 3).fill_(1.0f);
            if(background.has_value())
            {
                rgbt.narrow(3, 0, 3).copy_(background.value().reshape({1, 1, 1, 3}).to(at::kFloat));
            }
            return rgbt;
        }
        ...
        if(background.has_value())
        {
            state.background.narrow(1, 0, 3).copy_(background.value().unsqueeze(0).to(at::kFloat));
        ...
        // compatibility wrapper.  Output is float32 in the FP32 native path.
        at::Tensor rgbt = out_rgbt.has_value() ? out_rgbt.value() : at::empty({1, height, width, 4}, opts_f);
        ...
        else
        {
            ...
            // Pre-activated RGB: projection only, then copy colors (half or float)
            // into the FP32 state buffer so the rasterizer always consumes float.
            state.colors    = colors_packed.to(at::kFloat).contiguous();
            raster_colors   = &state.colors;
        }
        ...
        // ==================================================================
        // Return native float4 RGBT output (FP32)
        // ==================================================================
        return rgbt;
    }
    ...
    // ==========================================================================
    // ---------------------------------------------------------------------------
// GaussianInferenceRenderer::getVisibleMask
// ---------------------------------------------------------------------------
at::Tensor GaussianInferenceRenderer::getVisibleMask() const
{
    if (!state_ || state_->num_gaussians == 0) {
        return at::empty({0});
    }
    // The visible tensor is a bitmask stored as int32 [(N+31)/32]
    // We return it as-is for Python-side conversion to indices.
