...
public:
    ...
    // Debug-only recovery surface.  The returned tensors are native hierarchy
    // buffers, not timing-path instrumentation: [macro offsets, sorted local
    // Gaussian ids, macro batch offsets, active fine-tile masks].
    std::vector<at::Tensor> debug_intersection_tensors() const;
...
private:
    ...
    at::Tensor m_tileBuffer;          // [N] float — fixed-layout partial RGBT per (cta, mt-slot)
    at::Tensor m_activeMask;          // [total_ctas] uint32 — per-CTA 32-bit tile overlap mask
    int64_t m_tileBufferCapacity = 0; // high-water-mark for m_tileBuffer (in float elements)
