...
#include "csrc/gaussian_inference/GatherlessProjectedProducer.h"
#include "csrc/gaussian_inference/ProjectedStateAdapter.h"
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m)
{
    ...
    m.def(
        ...
        &gsplat::gaussian_render_inference_scene::higs_gatherless_projected_producer,
        "F9-1 direct master-indexed FP32 projection and SH3 producer.",
        py::arg("visible_ids"), py::arg("means"), py::arg("quats"),
        py::arg("scales"), py::arg("opacities"), py::arg("coeffs"),
        py::arg("viewmats"), py::arg("Ks"), py::arg("cam_positions"),
        py::arg("width"), py::arg("height"), py::arg("eps2d"),
        py::arg("near_plane"), py::arg("far_plane"), py::arg("radius_clip")
    );
    m.def(
        "higs_camera_positions_from_viewmats",
        &gsplat::gaussian_render_inference_scene::higs_camera_positions_from_viewmats,
        "Exact FP32 camera origins: -R^T t for [C,4,4] view matrices.",
        py::arg("viewmats")
    );

    m.def(
        "higs_native_hierarchy_from_projected",
        &gsplat::gaussian_render_inference_scene::higs_native_hierarchy_from_projected,
        "P2-1A-R2 native HiGS forward from F9 FP32 projected primitive state; never invokes packed inference projection.",
        py::arg("visible_ids"), py::arg("radii"), py::arg("means2d"), py::arg("depths"),
        py::arg("conics"), py::arg("opacities"), py::arg("colors"), py::arg("width"),
        py::arg("height"), py::arg("tile_size"), py::arg("background") = py::none(), py::arg("debug") = false
    );
...
{
    m.def(
        "higs_native_hierarchy_from_projected(Tensor visible_ids, Tensor radii, Tensor means2d, Tensor depths, "
        "Tensor conics, Tensor opacities, Tensor colors, int width, int height, int tile_size, Tensor? background=None, "
        "bool debug=False) -> (Tensor rgb, Tensor alpha, Tensor[] diagnostics)"
    );
    m.def(
...
{
    m.impl("higs_native_hierarchy_from_projected", &gsplat::gaussian_render_inference_scene::higs_native_hierarchy_from_projected);
