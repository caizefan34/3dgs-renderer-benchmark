...
void launch_macro_tile_rasterize(
    ...
    const at::Tensor &conics,              // [N, 4] float {l0,l1,l2,opacity}
    const at::Tensor &colors,              // [N, 4] float {R,G,B,0}
    ...
    at::Tensor &tile_buffer,               // fixed-layout partial RGBT (fp32)
...
void launch_macro_tile_post_blend(
    ...
    int32_t macro_tile_cols,            // macro-tile grid width
    const at::Tensor &mt_batch_offsets, // [n_mt+1] gaussian batch start indices
    const at::Tensor &tile_buffer,      // fixed-layout partial RGBT (fp32)
    const at::Tensor &active_mask,      // [total_ctas] uint32
    const at::Tensor &backgrounds,      // [4] float {R,G,B,pad}
    uint32_t image_width,
    uint32_t image_height,
    at::Tensor &render_colors
); // [H, W, 4] float {R,G,B,T} output
} // namespace higs
