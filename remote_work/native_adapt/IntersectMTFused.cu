...
__global__ void __launch_bounds__(MT_CTA_THREADS, MT_MIN_BLOCKS) fused_mt_binning_kernel(
    ...
    const float *__restrict__ conics,                // [N, 4] float {l0,l1,l2,opacity}
...
)
{
    ...
    {
        m_tileBuffer         = at::empty({max_tile_buf_size}, at::TensorOptions().dtype(at::kFloat).device(at::kCUDA));
...
std::vector<at::Tensor> IntersectMTFused::debug_intersection_tensors() const
{
    return {m_mtGaussOffsets, m_mtGaussIdsSorted, m_mtBatchOffsets, m_activeMask};
}
