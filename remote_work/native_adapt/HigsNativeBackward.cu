...
__global__ void __launch_bounds__(256, 5) higs_blend_bwd_kernel(
    const uint32_t I,
    const uint32_t N, // visible rows; parameters below are master tensors
...
// ============================================================================
template<uint32_t CDIM, uint32_t PX>
...
__global__ void higs_projection_bwd_kernel(
    ...
    const float *__restrict__ scales,   // [M, 3]
    ...
    const vec2 *__restrict__ v_means2d, // [I * N, 2]
...
)
{
    ...
    mat2 v_covar2d_inv
        = mat2(v_conic_ptr->x, v_conic_ptr->y * 0.5f, v_conic_ptr->y * 0.5f, v_conic_ptr->z);
    mat2 v_covar2d(0.f);
    ...
    // F9-1: read master parameters directly; no compact visible tensors.
    const int64_t m_id = visible_ids[g];
    const vec4 quat  = glm::make_vec4(quats + m_id * 4);
    const vec3 scale = glm::make_vec3(scales + m_id * 3);
    mat3 covar;
    quat_scale_to_covar_preci(quat, scale, &covar, nullptr);
    const vec3 mean_w = glm::make_vec3(means + m_id * 3);
    vec3 mean_c;
...
__global__ void higs_sh_vjp_grid_kernel(
    ...
    const float *__restrict__ coeffs,         // [M, K, D]
...
)
{
    ...
    if(!masked)
    {
        ...
        const vec3 dir = glm::make_vec3(means + m_id * 3) - cam_pos;

        const float *coeff_ptr    = coeffs + m_id * K * D;
...
)
{
    ...
    // `means`/`quats`/`scales` are master tensors in F9-1.  All projected
    // state remains compact, so N is the visible-domain extent.
...
#define LAUNCH_BLEND_BWD(CDIM)                                                                     \
    {                                                                                              \
        ...
        {                                                                                           \
            C10_CUDA_CHECK(cudaFuncSetAttribute(higs_blend_bwd_px_kernel<(CDIM), 1>,                \
                cudaFuncAttributeMaxDynamicSharedMemorySize, static_cast<int>(shmem_size)));        \
            higs_blend_bwd_px_kernel<(CDIM), 1><<<grid, threads_px, static_cast<size_t>(shmem_size), stream>>>( \
                static_cast<uint32_t>(I), static_cast<uint32_t>(N), static_cast<uint32_t>(n_isects),\
                reinterpret_cast<const vec2 *>(means2d.const_data_ptr<float>()),                    \
                reinterpret_cast<const vec3 *>(conics.const_data_ptr<float>()),                     \
                colors_eval.const_data_ptr<float>(), opacities.const_data_ptr<float>(),             \
        ...
        {                                                                                           \
            C10_CUDA_CHECK(cudaFuncSetAttribute(higs_blend_bwd_px_kernel<(CDIM), 2>,                \
                cudaFuncAttributeMaxDynamicSharedMemorySize, static_cast<int>(shmem_size)));        \
            higs_blend_bwd_px_kernel<(CDIM), 2><<<grid, threads_px, static_cast<size_t>(shmem_size), stream>>>( \
                static_cast<uint32_t>(I), static_cast<uint32_t>(N), static_cast<uint32_t>(n_isects),\
                reinterpret_cast<const vec2 *>(means2d.const_data_ptr<float>()),                    \
                reinterpret_cast<const vec3 *>(conics.const_data_ptr<float>()),                     \
                colors_eval.const_data_ptr<float>(), opacities.const_data_ptr<float>(),             \
        ...
        {                                                                                           \
            C10_CUDA_CHECK(cudaFuncSetAttribute(higs_blend_bwd_px_kernel<(CDIM), 4>,                \
                cudaFuncAttributeMaxDynamicSharedMemorySize, static_cast<int>(shmem_size)));        \
            higs_blend_bwd_px_kernel<(CDIM), 4><<<grid, threads_px, static_cast<size_t>(shmem_size), stream>>>( \
                static_cast<uint32_t>(I), static_cast<uint32_t>(N), static_cast<uint32_t>(n_isects),\
                reinterpret_cast<const vec2 *>(means2d.const_data_ptr<float>()),                    \
                reinterpret_cast<const vec3 *>(conics.const_data_ptr<float>()),                     \
                colors_eval.const_data_ptr<float>(), opacities.const_data_ptr<float>(),             \
...
#undef LAUNCH_BLEND_BWD
    ...
    // ---- Stage 2: projection VJP ----
    {
        ...
        higs_projection_bwd_kernel<<<grid, threads, 0, stream>>>(
            static_cast<uint32_t>(I),
            ...
            reinterpret_cast<const vec3 *>(conics.const_data_ptr<float>()),
            reinterpret_cast<const vec2 *>(v_means2d.const_data_ptr<float>()),
