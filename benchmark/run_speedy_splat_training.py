#!/usr/bin/env python
"""Run one audited from-SfM Speedy-Splat job for the HiGS paper protocol.

The official j-alex-hanson/speedy-splat train.py runs as a subprocess at the
protocol's fixed commit (plus the audited seed patch). Wall-clock markers are
captured from training.log at the 7k/15k/30k eval+save points, and each
checkpoint is scored on the official test split (every 8th image) with the
...
from higs_training_commands import build_speedy_splat_invocation  # noqa: E402

EVAL_SCRIPT = ROOT / "src" / "scripts" / "eval_speedy_splat_checkpoint.py"
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--method", choices=("speedy_splat",), required=True)
...
def main() -> int:
    ...
    invocation = build_speedy_splat_invocation(
        ...
        # The official train.py binds network_gui on a fixed port; give each
        # concurrent GPU its own port so 8-way runs do not collide.
        gui_port=6009 + args.gpu_id,
    )
    ...
    if returncode != 0:
        ...
        raise SystemExit(f"Speedy-Splat training failed rc={returncode}\n{tail}")
