...
__global__ void intersect_tile_kernel(
    ...
    // C1: image_id shifted to above tile_id: iid << (16 + tile_n_bits)
    // Baseline was: iid << (32 + tile_n_bits)
    ...
    // C1: keep upper 16 bits of depth (exponent + top mantissa bits)
    ...
    for (int32_t i = tile_min.y; i < tile_max.y; ++i) {
        for (int32_t j = tile_min.x; j < tile_max.x; ++j) {
            ...
            // Baseline: 32 + tile_n_bits + image_n_bits bits (47 for tile16 1080p)
            // Result: 12 → 8 CUB radix passes (33% fewer)
...
__global__ void intersect_offset_kernel(
    ...
    // C1: shift depth out (now only 16 bits at bottom)
...
void radix_sort_double_buffer(
    ...
    CUB_WRAPPER(
        ...
        n_isects,
        0,
        16 + tile_n_bits + image_n_bits, // C1: depth reduced from 32 to 16 bits
        at::cuda::getCurrentCUDAStream()
    );
...
void segmented_radix_sort_double_buffer(
    ...
    CUB_WRAPPER(
        ...
        0,
        16 + tile_n_bits, // C1: depth reduced from 32 to 16 bits
        at::cuda::getCurrentCUDAStream()
    );
    switch (d_keys.selector) {
    case 0: // sorted items are stored in isect_ids
