"""Focused tests for the Phase-12 30k-budget gradient-accumulation protocol.

Pre-registered exploration matrix exploration_accel12_11s0: 5 methods x 11
scenes x seed 0 = 55 jobs. accel12 tests Faster-GS-style gradient accumulation
(higs_accum_steps 2/4) at the FULL 30k budget -- accel7 only ran accum8 at 27k
where it starved densification -- as an orthogonal quality-safe lever that
amortizes the optimizer segment (~12% of step time in accel10 segment timing),
then composes it with accel11's full-res SSIM every-2 and accel10's post-refine
backward gating. All patched methods reuse the audited accel10 source tree
(same patch sha256), so no second CUDA build is needed. The accum flush logic
forces an optimizer step before every densification / opacity-reset boundary,
so accumulated gradient information is never dropped at topology changes.
"""
import json
import sys
import unittest
...
ACCEL12_METHODS = [
    ...
    "gsplat_30k_accum2",
    "gsplat_30k_accum4",
    "higs_skipbwd_30k_ssim_e2_accum2",
    "higs_skipbwd_30k_ssim_e2_accum4",
]
...
class HigsAccel12ProtocolTest(unittest.TestCase):
    ...
    def test_iterations(self):
        expected = {
            "gsplat_27k": 27000,
            "gsplat_30k_accum2": 30000,
            "gsplat_30k_accum4": 30000,
            "higs_skipbwd_30k_ssim_e2_accum2": 30000,
            "higs_skipbwd_30k_ssim_e2_accum4": 30000,
        }
    ...
    def test_accel12_lever_configs(self):
        cfg = self.protocol["methods"]["gsplat_30k_accum2"]["algorithm"]["trainer_cfg"]
        ...
        cfg4 = self.protocol["methods"]["gsplat_30k_accum4"]["algorithm"]["trainer_cfg"]
        self.assertEqual(cfg4["higs_accum_steps"], 4)
        cand = self.protocol["methods"]["higs_skipbwd_30k_ssim_e2_accum2"]["algorithm"]["trainer_cfg"]
        self.assertEqual(cand["higs_accum_steps"], 2)
        self.assertEqual(cand["higs_ssim_every"], 2)
        self.assertEqual(cand["higs_ssim_scale"], 1.0)
        self.assertEqual(cand["higs_skip_bwd_start_step"], 15000)
        cand4 = self.protocol["methods"]["higs_skipbwd_30k_ssim_e2_accum4"]["algorithm"]["trainer_cfg"]
        self.assertEqual(cand4["higs_accum_steps"], 4)
        self.assertEqual(cand4["higs_ssim_every"], 2)
        self.assertEqual(cand4["higs_skip_bwd_start_step"], 15000)
